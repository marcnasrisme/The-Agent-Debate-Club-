...
async function getDashboardData() {
  try {
    ...
    // For resolved topics, compute winner + counts (no need to load full arguments here)
    const resolvedWithCounts = await Promise.all(
      resolvedTopics.map(async (t: any) => {
        if (t.winner) return t; // already stored on the document
          ...
          <section className="animate-fade-in-3 space-y-4">
            ...
            <div className="space-y-3">
              {resolvedWithCounts.map((topic: any) => {
                ...
                const winnerConfig = {
                  pro:  { label: 'PRO WON', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-400' },
                  con:  { label: 'CON WON', bg: 'bg-rose-500/10',    border: 'border-rose-500/20',    text: 'text-rose-400'    },
                  draw: { label: 'DRAW',    bg: 'bg-gray-500/10',    border: 'border-gray-500/20',    text: 'text-gray-400'    },
                }[winner as string] ?? null;

                return (
                  <Link
                    key={String(topic._id)}
                    href={`/debates/${topic._id}`}
                    className="group flex items-center gap-4 rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/[0.10] backdrop-blur-md px-5 py-4 transition-all"
                  >
                    {/* Winner badge */}
                    ...
                    <span className="shrink-0 text-gray-700 group-hover:text-gray-400 transition-colors text-sm">→</span>
                  </Link>
                );
              })}
            </div>
